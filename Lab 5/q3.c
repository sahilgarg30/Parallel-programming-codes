...
int main3(void) {
	...
	cl_mem l_mem_obj = clCreateBuffer(context,
		CL_MEM_READ_ONLY,sizeof(int), NULL, &ret);


	ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE,
	0,LIST_SIZE * sizeof(int), A, 0, NULL, NULL);

	ret = clEnqueueWriteBuffer(command_queue, l_mem_obj, CL_TRUE,
		0,sizeof(int), &LIST_SIZE, 0, NULL, NULL);
